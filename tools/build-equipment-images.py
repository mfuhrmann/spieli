...
# Thumbnail width. Matches the ?width=800 the old Special:FilePath URLs asked
# for, so the rendered size does not change.
THUMB_WIDTH = 800
...
def main():
    ...
    if args.check:
        ...
        if current != text:
            print('\nERROR: %s is out of date. Run `make equipment-images`.' % OUT,
                  file=sys.stderr)
            return 1
        print('  ok  %s is up to date' % OUT)
        return 0

...
| `CompletenessLegend.svelte` | Map legend explaining the mapping-detail steps and the photo glyph |
...
The `playground_stats` materialised view is rebuilt with each `make db-apply` and carries the per-playground `completeness` (`'complete' | 'partial' | 'missing'`) plus its two inputs `has_equipment` / `has_info` and the separate `has_photo` signal — the rule mirrors `app/src/lib/completeness.js` exactly. The db-smoke workflow's "Assert JS/SQL rule parity" step guards the mirror: it reads `has_equipment` straight out of the live view definition and fails if it stops counting play infrastructure, starts counting street furniture again (#776), or if the classification CASE moves off `has_equipment AND has_info`. The job is path-filtered, so `app/src/lib/completeness.js` is listed as a trigger alongside `importer/api.sql`.

**Mapping detail** (`completeness`) = `has_equipment AND has_info` → `complete`, either one → `partial`, neither → `missing`. `has_equipment` counts play infrastructure only — `playground=*` objects plus soccer/basketball/table-tennis pitches. Street furniture (bench, shelter, picnic table) and the derived flags (`is_water`, `for_baby`, `for_toddler`, `for_wheelchair`) are deliberately excluded: they let a lone bench carry an otherwise unmapped playground. A photo is **not** an input either; it is an additive marker — a camera glyph on the map, nothing in the panel (the panel already shows the photos). The three identifiers are wire values and deliberately differ from their user-facing labels — `complete` displays as "detailed", `partial` as "basic", `missing` as "no details yet" (`mappingDetail.*` in `locales/*.json`). Never rename the identifiers: mixed-version federation depends on them. See [`docs/reference/completeness.md`](docs/reference/completeness.md).
...
- `playgroundStyleFn` — playground polygon fill/stroke, colour-coded by mapping detail; appends a camera glyph when `hasPhotoSignal(props)` is true

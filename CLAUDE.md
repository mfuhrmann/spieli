...
| `urlFraming.js` | Whether an explicit region-URL framing (e.g. `/Lauterbach`) was applied on load — `null` (undecided / no region path) \| `true` (override resolved & framed) \| `false` (region path present but fell back). Written by `StandaloneApp`, read by `LocateButton` so auto-locate only suppresses GPS centering when a region framing actually took effect. |

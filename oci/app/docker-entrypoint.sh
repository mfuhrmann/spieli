...
SAFE_BASEMAP_UPSTREAM=${SAFE_BASEMAP_UPSTREAM%/}
...
if [ -z "$SAFE_BASEMAP_STYLE_URL" ] && [ -z "$SAFE_BASEMAP_URL" ] \
   && [ -f "$WEBROOT/basemap/style.local.json" ]; then
...
location /basemap/ {
    # Vendored assets first (style.json, fonts), upstream for the rest.
...
location @basemap_upstream {
    ...
    # sub_filter cannot operate on a compressed body, and the only thing it
    # rewrites here is a small JSON document.

...
EXT_PANORAMAX=$(ext_enabled PROXY_PANORAMAX)
...
if [ -n "$EXT_NOMINATIM$EXT_COMMONS$EXT_MANGROVE$EXT_PANORAMAX" ]; then
...
location ~ ^/ext/wikimedia/(?<wm_host>[a-z0-9-]+\.wikimedia\.org)/(?<wm_path>[^?]+\.(?:png|jpg|jpeg|gif|webp|svg))\$ {
    limit_except GET { deny all; }

    set \$ext_wm "https://\$wm_host";
    # \$is_args\$args is not optional: the API hands out thumbnail URLs with a
    # utm_* query string attached, and dropping it changes the URL Wikimedia
    # was asked about.
    proxy_pass \$ext_wm/\$wm_path\$is_args\$args;
${_ext_common}
...
if [ -n "$EXT_PANORAMAX" ]; then
    cat >> /etc/nginx/ext-locations.conf.new <<EXTPXEOF
# ── /ext/panoramax/ → api.panoramax.xyz (THUMBNAILS ONLY) ────────────────────
# The viewer is an iframe and is deliberately not proxied: serving a whole
# interactive third-party application from this origin would grant it
# same-origin privileges here, which is worse than the cross-origin iframe.
# Gating that iframe behind a click is #852.
location ^~ /ext/panoramax/ {
    if (\$uri !~ "^/ext/panoramax/api/pictures/[A-Za-z0-9-]+/thumb\.jpg\$") { return 404; }
    limit_except GET { deny all; }

    set \$ext_panoramax "https://api.panoramax.xyz";
    rewrite ^/ext/panoramax/(.*)\$ /\$1 break;
    proxy_pass \$ext_panoramax;
${_ext_common}
    # The thumb endpoint is a stable redirect to object storage, so follow it
    # here rather than handing the browser an absolute third-party Location.
...
EXT_PANORAMAX_URL="https://api.panoramax.xyz"
...
if [ -n "$EXT_PANORAMAX" ]; then
    EXT_PANORAMAX_URL="/ext/panoramax"
fi
...
window.APP_CONFIG = {
  ...
  panoramaxApiUrl:   '${EXT_PANORAMAX_URL}',

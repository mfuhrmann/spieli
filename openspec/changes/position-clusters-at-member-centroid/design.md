...
### D3 — Federation contract: grouping vs. position, with a known interim cost

`add-federated-playground-clustering` (archived) merges clusters across backends by aligning on the shared cell grid. That contract refers to *which playgrounds are grouped together*, not *where the dot is drawn*. Two backends running this proposal will still group their respective members into the same cells; each backend's per-cell position reflects *its own* members' centroid.

For the cross-backend merge case (two backends contributing to the same cell), the hub today treats incoming `lon`/`lat` as opaque and picks one — typically the last one written, deterministic but not aware of weights. **This proposal accepts that interim behaviour and does not require a hub change.** The visible consequence in hub mode: when a user toggles a backend on/off via the InstancePanel, the dot for a shared cell can jump between the two backends' centroids. The dot is always *somewhere within the cluster's footprint* (not on a grid lattice), so the geographic-reading goal of this proposal is met even in the merge case; the jump is a residual jitter, not a wrong position.

A correctness-improving follow-up — re-computing a count-weighted mean across backends in the hub merger — is tracked separately (see Open Questions) and is out of scope here because it touches federation merge code, has its own test surface, and is not necessary to deliver the user-visible benefit.

...
**D5 — `playground=sandpit` counts, but only on `wheelchair=yes`.**
This reverses the original decision, which excluded sandpits outright and reasoned as D1 does about the area tag: an accompanying adult can lift a child into essentially any sandpit, so `wheelchair` there is true nearly everywhere and separates nothing.

That reading is wrong, and the wiki says so. `Key:playground` documents 87 values, none of which expresses a raised, roll-under sand table — and the same page instructs mappers to record equipment accessibility as `wheelchair=yes/no/limited`, "if the equipment can be used by wheelchair users". A mapper standing in front of a sand table therefore has exactly one way to record it: `playground=sandpit` + `wheelchair=yes`. The tag is not a claim about reaching the rim; it is the only expression the vocabulary permits for the feature that matters. Excluding it discards the signal it was meant to carry.

`limited` and `no` on a sandpit stay out — and, unlike before, out of `wheelchair_surveyed` as well. There the ambiguity is genuine: `limited` may mean "sand table, usable with assistance" or "you can get to the rim". Counting it as *surveyed* while it cannot count as *suitable* turns an unclear tag into a negative finding, which is the one reading it cannot support. Ignored on both sides, such a playground falls through to `NULL` — unknown, which is honest.

Measured on the current dataset: 36 sandpits, 5 carrying a `wheelchair` tag — 3 `no`, 2 `limited`, **0 `yes`**. So this changes no playground to `'yes'` today. It removes one false `'no'`: a playground whose only wheelchair-tagged device is a `limited` sandpit was reported as "surveyed, nothing suitable" on the strength of a tag saying the opposite. Distribution 849/59/18 → 850/58/18 (NULL/no/yes).

(The earlier note here cited "40 wheelchair-tagged sandpits", none carrying `height` or a description. The re-measurement above says 5 of 36; the original figure appears to have counted sandpits rather than tagged ones. The absence of `height` still holds and is why `limited` cannot be disambiguated.)

...
- [x] Add a wheelchair-accessible badge to `PlaygroundPanel.svelte` status row, gated on `attr.for_wheelchair`, using the `Accessibility` lucide icon; add `details.wheelchairAccessible` to all four locale files (de/en active, fr/es maintained). Build + `make docker-build` verified; live locale chunks (de/en) carry the key, app serves 200.
- [x] PR: label `requires-schema-update`; note the `missing`→`partial` completeness ripple in the description. (PR #728)

...
- **Framing reworked**: "Data Quality" → "Mapping detail"; `high` / `medium` / `low` → `detailed` / `basic` / `no details yet`. The value describes the map, not the playground.
- **Palette reworked**: the diverging traffic light (green / yellow / red) is replaced by a green ramp ending in a cool slate — bright green → dark green → slate blue-grey. The ramp reads as "more / less" rather than "good / bad"; the slate step reads as "nothing here yet, help out" and links naturally into `DataContributionModal`. Ordered by visual weight rather than lightness, so the most detailed playgrounds are what stands out.

> **How this actually shipped.** The PR A/B/C plan below was abandoned partway.
> PR A ([#803](https://github.com/mfuhrmann/spieli/pull/803)) and the combined
> PR ([#804](https://github.com/mfuhrmann/spieli/pull/804)) were closed without
> merging, and the work was re-cut along a different seam — by *what the value
> means* versus *how the map paints it*:
>
> | | |
> |---|---|
> | [#885](https://github.com/mfuhrmann/spieli/pull/885) **merged** | the rule, the wording, the camera glyph, the JS/SQL parity check — and #803's persisted criterion columns, folded in |
> | [#886](https://github.com/mfuhrmann/spieli/pull/886) *open* | the shared palette, the ring casing, every colour decision |
> | [#884](https://github.com/mfuhrmann/spieli/pull/884) **merged** | a prerequisite found on the way: the db-smoke job never applied `api.sql` and could not fail, so the parity check had nothing to run against |
>
> Section numbering below is left as written so the reasoning stays traceable;
> the PR each task landed in is what the table above says, not what the heading
> says.
...
- [~] 2.9 ~~Add the photo badge to `PlaygroundPanel`~~ — **dropped deliberately.** The panel already shows the photos themselves (CommonsGallery / PanoramaxViewer), so a chip saying "has a photo" tells the reader nothing they cannot see. The marker earns its place on the map, where it is the only way to tell without opening a playground
...
- [x] 2.16 Opened with the `requires-schema-update` label and the bucket-count shift in the body — as #885, not the planned PR B
...
- [x] 3.7 `i18n Guard` and the full suite green on #885
- [x] 3.8 Folded into #885 rather than opening a separate PR — the wording defines what the value *means*, so it belongs with the rule, not with the colours
...
- [x] 4.2 Hub mode smoke-tested locally against a real backend plus two stubs that omit the completeness fields (i.e. pre-P1 backends). Macro rings render; the unknown-completeness treatment is what drew attention to the violet ring covering the *whole* ring for those backends
